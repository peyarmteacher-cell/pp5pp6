    ...
    async function clearAllStudents() {
        ...
        try {
            ...
            if (result.message) {
                ...
                // If this is in academic management, it might be different, but let's assume it's here
                if (typeof loadAcademicYears === 'function') loadAcademicYears();
            } else {
                alert(result.error || 'เกิดข้อผิดพลาดไม่ทราบสาเหตุ');
            }
        } catch (e) {
            console.error('Error clearing students:', e);
            alert('เกิดข้อผิดพลาด: ไม่สามารถติดต่อเซิร์ฟเวอร์ได้ หรือรูปแบบข้อมูลผิดพลาด');
        }
    }
